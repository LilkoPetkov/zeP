...
pub fn doctor(
    ...
    var read_manifest = try ctx.manifest.readManifest(
    ...
    const manifest_root_json = try std.json.Stringify.valueAlloc(ctx.allocator, read_manifest.value, .{});
    ...
    if (manifest_from_lock == manifest_main) {
        try ctx.printer.append("Lock root matches zep.json.\n", .{}, .{ .color = .green });
    } else if (fix_issues) {
        try ctx.printer.append("Lock file schema root is not matching with zep.json.\n", .{}, .{ .color = .red });
        lock.value.root = read_manifest.value;
        try ctx.manifest.writeManifest(
            Structs.ZepFiles.PackageLockStruct,
            Constants.Extras.package_files.lock,
    ...
    if (!missing_packages and lock.value.packages.len == read_manifest.value.packages.len) {
        try ctx.printer.append("Lock file packages match exactly with zep.json!\n\n", .{}, .{ .color = .green });
    } else if (fix_issues) {
        try ctx.printer.append("Lock file packages mismatch with zep.json.\nzep.lock has priority!\n", .{}, .{ .color = .red });

        var pkg = try std.ArrayList([]const u8).initCapacity(ctx.allocator, 20);
        defer pkg.deinit(ctx.allocator);

        for (lock.value.packages) |lock_package| {
            try pkg.append(ctx.allocator, lock_package.name);
        }
