...
pub const Manifest = struct {
    ...
    pub fn addPathToManifest(
        ...
        for (package_manifest.value.packages) |p| {
            if (std.mem.eql(u8, p.name, package_id)) {
                for (p.paths) |path| try list_path.append(
                    self.allocator,
                    path,
                );
                continue;
        ...
        try list.append(
            self.allocator,
            Structs.Manifests.PackagePaths{
                .name = package_id,
    ...
    pub fn removePathFromManifest(
        ...
        if (list_path.items.len > 0) {
            try list.append(
                self.allocator,
                Structs.Manifests.PackagePaths{ .name = package_id, .paths = list_path.items },
            );
        } else {
